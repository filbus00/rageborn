...
- `PlayerHealth` (on the `Player Vitals` object, with `DeathFlow`) holds a `LifePool`, applies armor and raises `Died`. Life is carried between scenes in `GameSession`, so stairs do not heal. There are no potions or regeneration yet, so life only goes down until death.
...
- Items are pure data (`Item`: slot, rarity, item level; only the weapon slot exists). There are no affixes yet, so a weapon's damage depends on its item level alone and rarity is cosmetic. Affixes, the other slots and the equip screen come with the itemization step.
- `LootRoller` (pure, seeded, tested) follows `Docs/03-itemization.md`: a normal enemy drops an item 6 percent of the time, rarity weights 60, 30, 8.5 and 1.5, Magic Find multiplies the top three, and the legendary weight doubles after 300 kills without one and triples after 600 (every kill counts). Gold is `0.6 * level^1.3` rounded, with a floor of 1. Item level equals the enemy's level. The live session seeds its roller from the clock; pass a seed to replay.
...
- `GameSession.PickUp` equips a found weapon that beats the worn one and keeps the old one in the backpack. That automatic rule stands in for the equip screen. Consequence to know about: a character that dies has its equipped weapon left on the corpse, keeps the backpack and cannot equip a spare from it until an equip screen exists.
- The corpse now holds the real `Item`. Retrieving it equips it when the character is unarmed or wears something worse, otherwise moves it to the backpack, and leaves the corpse in place if the backpack has no room.
- `LootHud` is a placeholder text readout of gold, the bag and the weapon. Building its text allocates about 220 bytes, so it refreshes at most four times a second; the real HUD must format numbers without allocating.
- `Tools > ARPG > Add Loot To Sandbox` builds the beam, marker and coin art, the `Loot System` object and the readout. Build order is now: Create Sandbox Scene, Add Player And Camera, Add Enemies, Add Test Room, Add Combat, Add Survival, Add Loot, Create Town Scene.
- Not built yet: affixes, other slots, the inventory and equip screens, loot filter, salvage, elite and boss and chest sources, keeping drops across scene changes, pickup and drop sounds.
